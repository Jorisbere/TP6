...
$profile      = 'salaried'; // profil fiscal

$tax_due = calculateTax($profile, $income, $expenses, $dependents);

try {
    // Enregistrement dans la table declarations
    $stmt = $pdo->prepare("
    ...
    $pdf->Cell(0, 10, 'Declaration fiscale - Salarie', 0, 1, 'C');
    ...
    $pdf->Cell(0, 10, "Depenses : " . number_format($expenses, 0, ',', ' ') . " FCFA", 0, 1);
    $pdf->Cell(0, 10, "Personnes a charge : $dependents", 0, 1);
    $pdf->Cell(0, 10, "Impot estime : " . number_format($tax_due, 0, ',', ' ') . " FCFA", 0, 1);

    // Sauvegarde du PDF
    $fileName = "declaration_salaried_" . time() . ".pdf";
    $filePath = "../documents/" . $fileName;
    $pdf->Output('F', $filePath);

    // Enregistrement du PDF dans pdf_documents
    $pdfStmt = $pdo->prepare("

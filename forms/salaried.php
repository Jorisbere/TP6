...
<head>
    <meta charset="UTF-8">
    <title>Déclaration Salarié</title>
...
<div class="container">
    <h2>📄 Formulaire de déclaration – Salarié</h2>

    <!-- Sélection du déclarant -->
    <form method="GET" action="salaried.php" style="margin-bottom: 20px;">
    ...
    <!-- Formulaire fiscal -->
    <form method="POST" action="submit_salaried.php">
        <input type="hidden" name="declarant_id" value="<?= $selectedDeclarant['id'] ?? '' ?>">
        <!-- <div style="margin-bottom: 20px;">
  <a href="ajouter_declarant.php" style="text-decoration: none; background-color: #0078D7; color: white; padding: 10px 16px; border-radius: 6px; display: inline-block;">
    ➕ Ajouter un déclarant
        ...
        <label for="income">💰 Revenu annuel brut :</label>
        <input type="number" step="0.01" name="income" id="income" required min="0" placeholder="Ex: 12000.00">

        <label for="dependents">👨‍👩‍👧 Personnes à charge :</label>
        <input type="number" name="dependents" id="dependents" min="0" max="10" placeholder="Ex: 2">

        <label for="expenses">📉 Dépenses déductibles :</label>
        <input type="number" step="0.01" name="expenses" id="expenses" min="0" placeholder="Ex: 1500.00">
